...
signal updated(source, state, delta) # Time to update(based on process_mode), up to user to handle any logic, for example, update movement of KinematicBody
...
func _on_updated(source, state, delta):
	pass
...
func update(source: String, delta=get_physics_process_delta_time()):
	if not active:
		return
	...
	_on_updated(source, current_state, delta)
	emit_signal("updated", source, current_state, delta)
	if update_process_mode == UpdateProcessMode.MANUAL:
